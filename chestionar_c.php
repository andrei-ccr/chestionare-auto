<?php
	...
	if($_SESSION['chestionar_alc'] != true) {
		...
		$query = "SELECT id FROM intrebari_c ORDER BY id DESC LIMIT 1";
	...
	if($_SESSION['normal']) {
		$query = "SELECT * FROM intrebari_c WHERE id={$_SESSION['id_intreb'][$_SESSION['intrebare']]} LIMIT 1";
	}
					
	elseif($_SESSION['normal'] == false) {
		$query = "SELECT * FROM intrebari_c WHERE id={$_SESSION['id_intreb'][$_SESSION['intrebare2']]} LIMIT 1";
	}	
